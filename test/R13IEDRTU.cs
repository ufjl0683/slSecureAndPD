 ...
 //   public delegate void OnCommStateChangedHandler(IRTU sender,int comm_state);
    public  class R13IEDRTU : ModbusTCP.IRTU 
    {
      //  public event RegisterValueChangedHandler OnRegisterValueChangeEvent;
        ...
        public R13IEDRTU(string ControlID, int DevID, string IP, int Port, int StartAddress, int RegisterLength, int comm_state)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (RTUDevice != null && RTUDevice.connected)
                    {
                        lock (lockobj)
                        {
                            RTUDevice.ReadInputRegister((ushort)this.DevID, (byte)this.DevID, (ushort)(StartAddress - 1), this.RegisterLength, ref tempdata);
